//
//  Constants.swift
//  Weather
//
//  Created by minh duc on 2/18/19.
//  Copyright © 2019 minhduc. All rights reserved.
//

import UIKit

struct Constants {
    static let redirectionError = "It was transferred to a different URL. I'm sorry for causing you trouble"
    static let clientError = "An error occurred on the application side. Please try again later!"
    static let serverError = "A server error occurred. Please try again later!"
    static let unofficialError = "An error occurred. Please try again later!"
    static let networkError = "An error occurred internet. Please try again later!"
    static let noData = "\tNo location found !"
    static let searchCell = "SearchLocationTableViewCell"
    static let ok = "OK"
    static let zeroResult = "ZERO_RESULTS"
    static let limitQuery = "OVER_QUERY_LIMIT"
    static let requestDenied = "REQUEST_DENIED"
    static let unknownError = "UNKNOWN_ERROR"
    static let invalidRequest = "INVALID_REQUEST"
    static let blank = ""
    static let warning = "Warning"
    static let detail = "DETAIL"
    static let hourly = "NEXT 24 HOURS"
    static let forecastday = "NEXT 7 DAYS"
    static let uvcondition = "UV index"
    static let hudimity = "HUMIDITY"
    static let visibility = "Visibility"
    static let pressure = "Pressure"
    static let windspeed = "Windspeed"
    static let sunset = "Sunset"
    static let sunrise = "Sunrise"
}
